...
function removeFirstOccurrences(str, value) {
  ...
  return str
    ...
    .concat(str.slice(startPosition + value.length));
}
...
function removeLastOccurrences(str, value) {
  ...
  const endPosition = str.lastIndexOf(value);
  return str
    .slice(0, endPosition)
    .concat(str.slice(endPosition + value.length));
}
...
function sumOfCodes(str) {
  return typeof str === 'string'
    ? str.split('').reduce((akk, letter) => akk + +letter.charCodeAt(), 0)
    : 0;
}
...
function countVowels(str) {
  ...
  return str
    .split('')
    .reduce((acc, letter) => acc + vowels.includes(letter), 0);
}
...
function encodeToRot13(str) {
  ...
  return str
    .split('')
    .map((item) => {
      const ind = alphabet.indexOf(item);
      return alphabet.includes(item) ? cipher[ind] : item;
    })
    .join('');
}
